...
class AudioNormalizerApp:
    ...
    def create_widgets(self):
        ...
        # Limits the file list to 6 rows to maintain a compact vertical layout
        ...
        # Limits the process log to 8 lines to preserve vertical space for bottom controls
        ...
        # Highlights the main action button using the accent style
        ...
        # Binds the status bar and progress bar to the main frame to ensure layout stability
    ...
    def start_task(self, task_type):
        ...
        processor = FFMpegProcessor(
            config.ffmpeg_path, 
            update_callback=lambda msg: self.gui_queue.put(("info", msg)),
    ...
    def task_runner(self, task_type, files, processor, lufs, tp, mode, mastering_preset):
        ...
        for i, file_path in enumerate(files):
            ...
            self.gui_queue.put(("status", get_text(f"status_{task_type}_running", file=base_name)))
            ...
            log_content = f"\n--- File: {file_path} ---\n{stderr}\n"
            self.gui_queue.put(("log", (log_file, log_content, "a")))
            ...
            if return_code != 0:
                ...
                self.gui_queue.put(("error", (error_title, error_msg)))
                return

            self.gui_queue.put(("progress", i + 1))
        
        self.gui_queue.put(("finish", "cancelled" if was_cancelled else "completed"))
        ...
        try:
            while True:
                ...
                if task == "info": self.update_process_info(data)
                    ...
                    if core.app_logger: core.app_logger.log(*data)
    ...
    def show_options(self):
        ...
        # Horizontal container for side-by-side language and theme selection
