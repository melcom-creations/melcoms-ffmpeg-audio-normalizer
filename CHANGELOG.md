...
- **GitHub Release Update Notifications**
  - Added a non-blocking GitHub Releases check that can notify users when a newer public version is available.
  - Added an optional automatic check at application startup, enabled by default and configurable under File - Options.
  - Added an optional, disabled-by-default setting to include GitHub pre-releases in both automatic and manual checks.
  - Replaced the former website link under Info - Check for Updates with a real manual version check that always reports its result.
  - Automatic checks remain silent when the installed version is current or GitHub cannot be reached.
  - Updates are never downloaded or installed automatically; the notification only links to the matching GitHub release.
  - Added complete English, German, Polish, and Swedish localization and help documentation for the feature.

### Changed in Version 4.1.1

- **Modernized Options Dialog**
  - Replaced the stacked legacy layout with a spacious card-based design for appearance, FFmpeg, logging, and update settings.
  - Added a clear dialog header, concise setting descriptions, balanced two-column controls, and a dedicated action footer.
  - Added localized **Cancel** and **Save Changes** actions in all four supported languages.
- **Localization and Audio Terminology Review**
  - Reviewed all four language files for natural UI wording and audio-specific terminology.
  - Standardized terms for sample rate, bit depth, RMS level, metadata, dynamics, codecs, and related audio measurements where appropriate.

### Fixed in Version 4.1.1

- **License Filename Consistency**
...
- **Unsafe Integer and Boolean Parsing from INI**
  - Replaced direct `settings.getint()` and `settings.getboolean()` calls with safe wrapper methods `_get_int_safe()` and `_get_bool_safe()`.
  - Both wrappers catch `ValueError`, `TypeError`, and `configparser.Error`, returning the defined fallback value instead of raising an unhandled exception.
- **Unhandled Write Error in save_options()**
  - Wrapped the `options.ini` write operation in a `try/except OSError` block.
  - Prevents a crash when the application directory is read-only or the file cannot be written due to permission restrictions.
- **Metadata Loss When Saving Artwork**
  - Added `-map_metadata 0` to the FFmpeg artwork-save command, preventing global metadata from being silently discarded when embedding cover art into MP3, FLAC, and M4A files.
- **Ambiguous Stream Selection During Normalization**
  - Added explicit `-map 0:a` and `-map 0:v?` stream selectors to the normalization command for MP3, FLAC, and M4A output formats.
  - Prevents FFmpeg's default stream auto-selection from unexpectedly copying or omitting cover art or foreign video streams in multi-stream files.
- **Nested Exception Handling in save_track_metadata()**
  - Refactored the nested `try/except` structure in `save_track_metadata()` into a flat, readable layout using a single outer `try/finally` block.
  - Temp file cleanup is now guaranteed in all exit paths without code duplication.
- **Theme: Module Tracker - info_bg color updated**
  - Changed `info_bg` from `#000000` to `#B19172` for improved readability in the process log and inspector text areas.

---

## [4.1.0] (Hotfix) - 2026-06-13

### Kontrollraum Edition for Version 4.1.0 Hotfix

### Fixed in Version 4.1.0 Hotfix
...
### Kontrollraum Edition for Version 4.1.0
...
- **Drag & Drop File Queue Support**: Added native drag & drop support for the main file queue.
  ...
  - **Artwork Tab**: Added a brand-new tab for album covers. Allows users to view the embedded cover, check its resolution and codec (e.g., JPG / 1024 x 1024), export the image, change it, or delete it entirely via a dedicated "X" button. Cover modification and deletion operations are performed losslessly and are supported for MP3, FLAC, and M4A files.
...
### Additional Fixes in Version 4.0.3
...
### Changed in Version 1.0.0 - 1.4.0
