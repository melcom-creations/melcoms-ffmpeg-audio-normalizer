...
## 🛡 Important Hotfix Information (v4.1.0)

Shortly after the initial public release of v4.1.0, some users reported that Windows Defender could incorrectly flag `AudioNormalizer.exe` as:

`Program:Win32/Wacapew.A!ml`

After investigation, the detection was traced to the executable compression method used during packaging and not to the application itself.

The release package has since been rebuilt and republished as **v4.1.0 (Hotfix)** without the affected compression stage.

### What did this warning actually mean?

The reported detection was a heuristic machine-learning based classification generated by Microsoft Defender.

In simple terms, Defender did **not** identify a known virus, trojan, spyware component, cryptocurrency miner, ransomware payload, or any other malicious software inside the application. Instead, Defender considered certain characteristics of the packaged executable suspicious and classified it as potentially unwanted.

This type of detection can occasionally occur with legitimate software, especially when executable compression and packaging technologies are used.

### Important

* No malware, trojans, spyware, ransomware, cryptocurrency miners, backdoors, or other harmful components were ever present in the application.
* No malicious code has ever been included in the project's source code.
* Previous releases, including v4.0.3 and all earlier versions, were also clean.
* The Hotfix release only changes the packaging method used to create the executable.
* No functionality has been removed or altered.

If you downloaded the original v4.1.0 release, please replace it with the current **v4.1.0 (Hotfix)** package.
